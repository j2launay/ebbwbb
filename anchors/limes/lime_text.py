...
class TextDomainMapper(explanation.DomainMapper):
    ...
    def map_exp_ids(self, exp, vectorizer=None, positions=False):
        ...
        if positions:
            ...
            #print("les premières exp dans lime text", exp[:50])
            exp_final = []
            for i in range(25):
                try:
                    a = [i[0] for i in exp]
                    exp_final.append((self.indexed_string.word(i), exp[np.where(np.where(vectorizer.transform([self.indexed_string.word(i)])[0,:].toarray()[0])[0] == a)[0][0]][1]))
                    """print(self.indexed_string.word(i), exp[i][1])
                    print("numéro vectorizé", vectorizer.transform([self.indexed_string.word(i)]))
                    print("0 ligne np wheree", np.where(vectorizer.transform([self.indexed_string.word(i)])[0,:].toarray()[0])[0])
                    print("TEST", vectorizer.transform([self.indexed_string.word(i)])[:,0].toarray()[0])
                    print("value", exp[np.where(vectorizer.transform([self.indexed_string.word(i)])[0,:].toarray()[0])[0][0]])
                    a = [i[0] for i in exp]
                    print(np.where(np.where(vectorizer.transform([self.indexed_string.word(i)])[0,:].toarray()[0])[0] == a)[0])
                    print("value FINALE pour le mot", self.indexed_string.word(i), exp[np.where(np.where(vectorizer.transform([self.indexed_string.word(i)])[0,:].toarray()[0])[0] == a)[0][0]])
                    print()"""
                except:
                    pass            
            exp = exp_final#[(self.indexed_string.word(x[0]), x[1]) for x in exp]
        return exp
...
class IndexedString(object):
    ...
    def __init__(self, raw_string, split_expression=r'\W+', bow=False,
                 mask_string=None):
        """Initializer.
        Args:
...
class IndexedCharacters(object):
    ...
    def __init__(self, raw_string, bow=False, mask_string=None):
        """Initializer.
        Args:
            raw_string: string with raw text in it
...
class LimeTextExplainer(object):
    ...
    def explain_instance(self,
                         text_instance,
                         classifier_fn,
                         sentences_in_field=None,
                         distance_sentences=None,
                         labels_in_field=None,
                         labels=(1,),
                         top_labels=None,
                         num_features=10,
                         num_samples=5000,
        ...
        if sentences_in_field is not None:
            data = sentences_in_field
            yss = labels_in_field
            distances = distance_sentences
        else:
            data, yss, distances = self.__data_labels_distances(
                    indexed_string, classifier_fn, num_samples,
                    distance_metric=distance_metric)
            num_features = indexed_string.num_words()
        ...
        ret_exp.indexed_string = indexed_string
        return ret_exp
